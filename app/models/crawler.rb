...
client = Mysql2::Client.new(:host => "localhost", :username => "root", :password => 749136, :database => "asoft_test_development")
obj = Crawler.new
carusel_products = obj.get_carusel
others_products = obj.get_others_cat
products = carusel_products.merge(others_products)
products.each do |key,val|
	client.query("insert into products (category_name,content,created_at,updated_at) values('#{key}','#{val.to_json.to_s}','#{Time.now}','#{Time.now}')")
end